<?php
$koneksi     = mysqli_connect("localhost", "root", "", "showroom");
$bulan       = mysqli_query($koneksi, "SELECT bulan FROM penjualan WHERE tahun='2016' order by id asc");
$penghasilan = mysqli_query($koneksi, "SELECT hasil FROM penjualan WHERE tahun='2016' order by id asc");

$hitung = mysqli_query($koneksi, "SELECT MAX(hasil) AS hasil_penjualan FROM penjualan");
$hitung_penjualan = mysqli_fetch_array($hitung);
print_r($hitung_penjualan);
?>
<html>
    <head>
    ...
    options: {
      ...
      scales: {
        ...
        yAxes: [{
          ticks: {
            min: 0,
            max: <?php echo $hitung_penjualan["hasil_penjualan"]; ?>,
            maxTicksLimit: 5,
            padding: 10,
